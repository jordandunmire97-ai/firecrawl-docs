...
<Note>
  This page is the canonical path for AI-agent onboarding. For general API integration, use [Introduction](/introduction). For MCP client-specific setup, use [MCP Server](/mcp-server). For role-based routing, use [Start Here by Role](/start-here-by-role).
</Note>
