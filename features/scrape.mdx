...
When you call `/scrape`, Firecrawl processes the request through a multi-stage pipeline:

1. **Proxy routing** — Firecrawl selects a proxy based on your `proxy` setting (`"auto"`, `"basic"`, or `"enhanced"`). Automatic mode routes simple pages through a basic proxy and upgrades to an enhanced proxy when the site requires it. This is why the same URL can sometimes return faster or slower depending on what the target site demands.

2. **JavaScript rendering** — The page is loaded in a real browser. JavaScript executes, dynamic content renders, and single-page app frameworks (React, Vue, Angular, Next.js, etc.) resolve their client-side routes before content is extracted. This is why Firecrawl returns complete content where a basic `curl` or `fetch` would only return the HTML shell.

3. **Cache lookup** — Before scraping, Firecrawl checks whether a recent cached copy exists within your `maxAge` window (default: 2 days). If a valid cache hit is found, it is returned immediately, often reducing latency by up to 5×. Set `maxAge: 0` to bypass the cache and always fetch fresh content.

4. **Content extraction** — Once the page is rendered, Firecrawl extracts the requested formats. Markdown is generated by converting the rendered DOM into clean, LLM-ready text with boilerplate removed. Structured JSON runs an LLM extraction pass against the schema you provide. Screenshots capture the rendered viewport.

5. **Post-processing** — Optional post-processing steps apply: PII redaction, PDF page parsing, audio/video extraction, or question-answering. These each add a small amount of latency and consume additional credits.

Understanding this pipeline helps you choose the right settings. If a page blocks standard proxies, use `proxy: "enhanced"`. If you need the absolute latest content, set `maxAge: 0`. If you need structured fields, the `json` format runs after rendering, so dynamic content is always captured first.
