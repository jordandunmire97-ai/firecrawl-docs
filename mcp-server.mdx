...
<Note>
  This page is focused on MCP transport and client configuration. If you need the full AI-agent onboarding flow, use [Build with AI](/ai-onboarding). If you are starting with direct API usage, use [Introduction](/introduction). You can also route by persona in [Start Here by Role](/start-here-by-role).
</Note>
